...
def plot_2d_q(axis, name, numb_qy, omega_min, omega_max):
    ...
    # qy_list = []
    # if name == "V1":
    #     qy_list = [5, 6, 7, 8, 9, 10, 12, 13]
    # elif name == "coulomb":
    #     qy_list = [6, 8, 9, 12]

    qy_list = np.arange(numb_qy)

    # qy_list = np.array([0])

    for qy_value in qy_list:
        omega = []
        SR = []
        ...
        with open('/home/bart/PycharmProjects/response_functions/test/FQHETorusSpectralResponse/stripped_files/' + file, 'r') as csvfile:
            ...
            for row in plots:
                ...
                if name == "V1":
                    omega.append((float(row[0])+10)/6.33667)
                elif name == "coulomb":
...
if __name__ == "__main__":
    ...
    plt.savefig("/home/bart/PycharmProjects/response_functions/test/figures/sr_newest_fermions_TargetSpace_0_scaled.png", bbox_inches='tight', dpi=300)
    plt.show()
