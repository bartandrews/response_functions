...
if __name__ == "__main__":
    ...
    for i in np.linspace(0, 6, 61, endpoint=True):
        ...
        file.write(f"echo {SpectralResponse} {response_functions}/vectors/coulomb/n_{n_val:g}/fermions_torus_kysym_coulomb_n_{n_val:g}_2s_{3*n_val:g}_ratio_1.000000_ky_{ky_val:g}.0.vec --sr-omega-min {mid-std/(2**i):.6g} --sr-omega-max {mid+std/(2**i):.6g} --sr-epsilon {0.0001/(2**i):.6g} --sr-omega-interval {0.00001/(2**i):.6g} --sr-spectral-resolution 1E-5 --use-coulomb --coulomb-strength 1 -y 0\n")
    file.write("}\n")
