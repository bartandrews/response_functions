if __name__ == "__main__":
    ...
    file = open(f"coulomb_sr_script.sh", "w")
    file.write("#!/bin/bash\n\n")

    file.write("runs() {\n")
    for n_val in [8]:
        ky_val = n_val / 2 if n_val % 2 == 0 else 0  # corresponding ky value for ground state
        file.write(f"echo {SpectralResponse} {response_functions}/vectors_2/coulomb/n_{n_val:g}/fermions_torus_kysym_coulomb_n_{n_val:g}_2s_{3*n_val:g}_ratio_1.000000_ky_{ky_val:g}.0.vec --sr-omega-min -100 --sr-omega-max 100 --sr-epsilon 1E-4 --sr-omega-interval 1E-5 --sr-spectral-resolution 1E-5 --use-coulomb --coulomb-strength 1 -m 8000\n")
