...
def plot_2d_q(axis, name, numb_qy, omega_min, omega_max):
    ...
    for qy_value in range(numb_qy):
        omega = []
        SR = []
        file = f"fermions_torus_spec_resp_kysym_{name}_n_{int(numb_qy/3)}_2s_{numb_qy}_ratio_1.000000_qy_{qy_value}" \
    ...
    if name == "V1":
        leg = axis.legend(loc='upper right', handletextpad=0, borderpad=0.2, framealpha=1, edgecolor=None,
                          markerscale=5, ncol=10, labelspacing=0, columnspacing=0, bbox_to_anchor=(2.2, 1.4),
                          title='$q_y$')
...
if __name__ == "__main__":
    ...
    plot_2d_q(ax0, "V1", 24, omega_min=-100, omega_max=100)
    ...
    plt.savefig("/home/bart/Documents/papers/SR/sr_n_8.png", bbox_inches='tight', dpi=300)
    plt.show()
