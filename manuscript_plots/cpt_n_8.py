...
def plot_2d_cpt_variable_full(axis, numb_qy, omega_min, omega_max):
    ...
    for i in range(2, int(numb_qy/2)+1, 2):
    ...
    for i, name in enumerate(name_list):
        ...
        with open('/home/bart/PycharmProjects/response_functions/FQHETorusSpectralResponse/stripped_files/' + file, 'r') as csvfile:
            ...
            for row in plots:
                omega.append(float(row[0])+10-6.4332450205269005)
...
def plot_3d_cpt_variable_full(axis, numb_qy, omega_min, omega_max):
    ...
    axis.set_xticks(np.arange(1, int(numb_qy / 2), 2))
...
def plot_2d_cpt_variable_full_box(axis, numb_qy, omega_min, omega_max):
    ...
    for i in range(2, int(numb_qy/2)+1, 2):  # 40
        name_list += [f"coulomb_0_plus_CoulombPlane_trunc_{i}"]
        lbl += [i-1]
...
def plot_2d_cpt_variable_full_hist(axis, numb_qy, omega_min, omega_max):
    ...
    counts, xedges, yedges, im = axis.hist2d(lbl, omega, bins=(int(numb_qy/2)-1, 20), range=[[0.5, int(numb_qy/2)-0.5], None], cmap=plt.cm.inferno)
...
if __name__ == "__main__":
    ...
    plt.savefig("/home/bart/Documents/papers/SR/cpt_n_8.png", bbox_inches='tight', dpi=300)
    plt.show()
