...
def plot_3d_q(axis, name, numb_qy, omega_min, omega_max):
    ...
    if name == "V1":
        scale = 1  # laughlin_gap / coulomb_gap
    ...
    if axis == ax0:
        axis.set_xticks(np.arange(0, 18, 3))
    elif axis == ax1:
        axis.set_xticks(np.arange(0, 18, 3))
        axis.set_yticks(np.arange(0, 1.3, 0.3))
    ...
    if axis == ax0:  # V1
        axis.set_position(Bbox.from_bounds(0, 0.64, 0.42, 0.28))
    elif axis == ax1:  # Coulomb
        axis.set_position(Bbox.from_bounds(0.44, 0.64, 0.42, 0.28))
...
def plot_2d_lty_omega_mean_alpha(axis, numb_qy, omega_min_val, omega_max_val):
    ...
    axis.xaxis.set_major_formatter(FormatStrFormatter('$%g$'))
    axis.set_ylabel('$\mu_{\omega_\mathrm{peak}}$')
    axis.yaxis.set_major_formatter(FormatStrFormatter('$%g$'))
    ...
    leg = axis.legend(loc='center', handletextpad=0, borderpad=0.2, framealpha=1,
                      ...
                      bbox_to_anchor=(0.5, 1.4), title='$\log_{10}(\lambda)$')
...
def plot_2d_lty_omega_range_alpha(axis, numb_qy, omega_min_val, omega_max_val):
    ...
    for lamb_exp in [-4, -3, -2, -1, 0, 1, 2]:
        ...
        for lbl_val in lbl_values:
            ...
            range_val = np.std(omega[omega_set[0]:omega_set[-1]+1])
    ...
    axis.set_xlabel('$\\alpha$')
    axis.xaxis.set_major_formatter(FormatStrFormatter('$%g$'))
    axis.set_ylabel('$\sigma_{\omega_\mathrm{peak}}$')
    axis.yaxis.set_major_formatter(FormatStrFormatter('$%g$'))
...
def plot_2d_lty_omega_range_lambda(axis, numb_qy, omega_min_val, omega_max_val):
    ...
    axis.set_xlabel('$\\log_{10}(\\lambda)$')
    axis.xaxis.set_major_formatter(FormatStrFormatter('$%g$'))
...
def plot_2d_lty_omega_mean_lambda_finite(axis, omega_min_val, omega_max_val):
    ...
    axis.set_xlabel('$\\log_{10}(\\lambda)$')
    axis.xaxis.set_major_formatter(FormatStrFormatter('$%g$'))
    axis.set_ylabel('$\\log_{10}(\mu_{\omega_\mathrm{peak}})$')
    axis.yaxis.set_major_formatter(FormatStrFormatter('$%g$'))

    leg = axis.legend(loc='lower left', handletextpad=0, borderpad=0.2, framealpha=1,
                      edgecolor=None, markerscale=1, ncol=8, labelspacing=0, columnspacing=0, title='$N_f$')
    leg.get_frame().set_linewidth(0.5)
...
if __name__ == "__main__":
    ...
    super_grid = gridspec.GridSpec(2, 1, height_ratios=[1, 2], hspace=0.65)  # 0.55
    ...
    bottom_super_grid = gridspec.GridSpecFromSubplotSpec(2, 1, super_grid[1, 0], height_ratios=[2, 1], hspace=0.5)
    ...
    ax0 = plt.subplot(left_cell, projection='3d')
    plot_3d_q(ax0, "V1", 18, omega_min=-100, omega_max=100)
    ax1 = plt.subplot(right_cell, projection='3d')
    plot_3d_q(ax1, "coulomb", 18, omega_min=-100, omega_max=100)
    ...
    fig.text(0.05, 0.88, "(a) $V_1$", fontsize=12)
    fig.text(0.495, 0.88, "(b) Coulomb", fontsize=12)
    fig.text(0.11, 0.54, "(c)", fontsize=12)
    fig.text(0.495, 0.54, "(d)", fontsize=12)
    ...
    ax2.text(0.1, 0.2, "mean", fontsize=11, transform=ax2.transAxes)
    ax3.text(0.1, 0.2, "stdev", fontsize=11, transform=ax3.transAxes)
    ax6.text(0.875, 0.73, "$\\alpha=1$", fontsize=11, transform=ax6.transAxes)

    plt.savefig("sr_lty_new_2.png", bbox_inches='tight', dpi=300)
    plt.show()
